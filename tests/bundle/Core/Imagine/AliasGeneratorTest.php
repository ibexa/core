...
final class AliasGeneratorTest extends TestCase
{
    ...
    public function testGetVariationNotStoredHavingReferences(): void
    {
        ...
        $expectedFilters = [$reference2, $reference1, $variationName];
        $this->filterManager
            ...
            ->willReturnCallback(static function (
                $binaryArg,
                $filter
            ) use ($binary, &$expectedFilters) {
                self::assertSame($binary, $binaryArg);
                $expectedFilter = array_shift($expectedFilters);
                self::assertSame($expectedFilter, $filter);
