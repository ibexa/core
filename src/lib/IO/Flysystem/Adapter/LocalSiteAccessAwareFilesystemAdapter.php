 ...
 * @internal
 *
 * @see \League\Flysystem\Local\LocalFilesystemAdapter
 */
final class LocalSiteAccessAwareFilesystemAdapter implements FilesystemAdapter
{
    ...
    public function listContents(string $path, bool $deep): iterable
    {
        ...
        $iterator = $deep ? $this->listDirectoryRecursively($location) : $this->listDirectory(
            $location
        );
    ...
    public function createDirectory(string $path, Config $config): void
    {
        ...
        if (!mkdir($location, $permissions, true) && !is_dir($location)) {
    ...
    private function setPermissions(string $location, int $visibility): void
    {
        ...
        if (!@chmod($location, $visibility)) {
            ...
            throw UnableToSetVisibility::atLocation(
                $this->prefixer->stripPrefix($location),
                $extraMessage
            );
        }
    }
