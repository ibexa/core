...
class Handler implements BaseContentHandler
{
            ...
            $this->contentGateway->insertRelation(
                new RelationCreateStruct(
                    [
                        ...
                        'sourceFieldDefinitionId' => $relation['ibexa_content_link_contentclassattribute_id'],
                        'destinationContentId' => $relation['ibexa_content_link_to_contentobject_id'],
                        'type' => (int)$relation['ibexa_content_link_relation_type'],
                    ]
                )
            );
        }
    ...
    public function load($id, $version = null, array $translations = null)
    {
        ...
        $contentObjects = $this->mapper->extractContentFromRows(
            ...
            $this->contentGateway->loadVersionedNameData([[
                'id' => $id,
                'version' => $rows[0]['ibexa_content_version_version'],
            ]]),
            'ibexa_content_',
            $translations
        );
    ...
    public function loadContentList(array $contentIds, array $translations = null): array
    {
        ...
        foreach ($rawList as $row) {
            ...
            $idVersionPairs[$row['ibexa_content_id']] = [
                'id' => $row['ibexa_content_id'],
        ...
        foreach ($nameData as $nameDataRow) {
            $contentId = $nameDataRow['ibexa_content_name_contentobject_id'];
        ...
        $contentItemsRows = [];
        foreach ($rawList as $row) {
            $contentId = $row['ibexa_content_id'];
    ...
    public function loadVersionInfo($contentId, $versionNo = null): VersionInfo
    {
        ...
        $versionInfo = $this->mapper->extractVersionInfoListFromRows(
            $rows,
            $this->contentGateway->loadVersionedNameData([['id' => $contentId, 'version' => $rows[0]['ibexa_content_version_version']]])
        );
    ...
    public function loadVersionNoArchivedWithin(int $contentId, int $seconds): array
    {
        ...
        $archivedVersionNos = [];
        foreach ($rows as $row) {
            $archivedVersionNos[] = (int) $row['ibexa_content_version_version'];
        }
    ...
    public function loadDraftsForUser($userId)
    {
        ...
        $idVersionPairs = array_map(
            static function ($row) {
                return [
                    'id' => $row['ibexa_content_version_contentobject_id'],
                    'version' => $row['ibexa_content_version_version'],
                ];
            },
            $rows
