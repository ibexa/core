...
final class DoctrineDatabase extends Gateway
{
    ...
    public function loadAllLocationEntries(int $locationId): array
    {
        ...
        $query
            ...
            ->from($this->connection->quoteSingleIdentifier($this->table))
            ->where('action = :action')
    ...
    public function historizeBeforeSwap(string $action, int $languageMask): void
    {
        $query = $this->connection->createQueryBuilder();
        $query
            ->update($this->connection->quoteSingleIdentifier($this->table))
            ->set(
                'is_original',
                $query->createPositionalParameter(0, ParameterType::INTEGER)
            )
    ...
    {
        $query = $this->connection->createQueryBuilder();
        $query->update($this->connection->quoteSingleIdentifier($this->table));
    ...
    public function insertRow(array $values): int
    {
        ...
        $query = $this->connection->createQueryBuilder();
        $query->insert($this->connection->quoteSingleIdentifier($this->table));
        foreach ($values as $columnName => $value) {
            $query->setValue(
                $columnName,
                $query->createNamedParameter(
    ...
    public function loadUrlAliasData(array $urlHashes): array
    {
        ...
        foreach ($urlHashes as $level => $urlPartHash) {
            ...
            $query
                ...
                ->from($this->connection->quoteSingleIdentifier($this->table), $tableAlias);
    ...
    public function remove(string $action, ?int $id = null): void
    {
        ...
        $expr = $query->expr();
        $query
            ->delete($this->connection->quoteSingleIdentifier($this->table))
            ->where(
                $expr->eq(
                    'action',
                    $query->createPositionalParameter($action, ParameterType::STRING)
                ...
                $subQuery->expr()->eq(
                    ...
                    sprintf(
                        ...
                        $this->getDatabasePlatform()->getSubstringExpression(
                            $this->connection->quoteSingleIdentifier($this->table) . '.action',
                            '8'
                        ),
                        $this->getIntegerType()
                    )
    ...
    private function getUrlAliasesForLocation(int $locationId): array
    {
        ...
        $queryBuilder
            ...
            ->from($this->connection->quoteSingleIdentifier($this->table), 't1')
