            ...
            ->arrayNode('options')
                ...
                ->children()
                    ...
                    ->integerNode('grace_period_in_seconds')
                        ->info('Provide a value in seconds, when archived content is still accessible for users with access to current version. Prevents 500 error when accessed content is updated during request. Defaults to twice the PHP maximum execution time.')
                    ->end()
                ->end()
            ->end();
    }
